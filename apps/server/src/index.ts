...
import { GEMINI_API_KEY } from '@repo/backend/config'; // Adjust the path as needed
...
function fileToGenerativePart(imageBuffer: Buffer, mimeType: string): GenerativePart {
  return {
    inlineData: {
...
async function analyzeImage(imageBuffer: Buffer,dictOfVars: Record<string, any>): Promise<object[]> {
...
"Analyze the image content thoroughly and return your answer following these rules, including detailed intermediate steps, robust error handling, and actionable suggestions or next steps when applicable.\"";
  ...
  try {
    ...
    const imagePart: GenerativePart = fileToGenerativePart(imageBuffer, mimeType);
...
app.post('/api/analyze/ai', upload.single('image'), async (req: Request, res: Response) =>{
  try {
    ...
    const dictOfVars: Record<string, any> = req.body.dictOfVars ? JSON.parse(req.body.dictOfVars) : {};
